...
import loaders.load_pivoting as pivoting
...
def get_args():
    ...
    ap.add_argument('-d', '--delta', type=float, default=0.5)
    ap.add_argument('-w', '--workers', type=int, default=4)
    ap.add_argument('-T', '--threads', type=int, default=1)
    ap.add_argument('-e', '--encoder', choices=['GCN', 'GAT', 'SAGE'], type=str.upper, default="GCN")
    ...
    ap.add_argument('-t', '--tests', type=int, default=5)
    ...
    args.te_end = None
    assert 0 <= args.fpweight <= 1, '--fpweight must be a value between 0 and 1 (inclusive)'

    readable = str(args)
    print(readable)

    model_str = f'{args.encoder} -> {args.rnn} ({args.impl})'
    print(model_str)
    ...
    if args.encoder == 'GCN':
        args.encoder = detector_gcn_rref if args.impl[0] == 'D' else predictor_gcn_rref
    elif args.encoder == 'GAT':
        args.encoder = detector_gat_rref if args.impl[0] == 'D' else predictor_gat_rref
    else:
        args.encoder = detector_sage_rref if args.impl[0] == 'D' else predictor_sage_rref
...
if __name__ == '__main__':
    ...
    for m in dfs['Model'].unique():
        ...
        compressed = pd.DataFrame(
            [numeric_df.mean(), numeric_df.sem()],  # Compute stats only on numeric columns
            index=['mean', 'stderr']
